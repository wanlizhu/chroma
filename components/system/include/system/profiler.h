...
public:
    ...
    public:
        ...
        std::chrono::duration<uint64_t, std::nano> get_wall_time() const {
            return std::chrono::duration<uint64_t, std::nano>(time_enabled);
        }

        std::chrono::duration<uint64_t, std::nano> get_running_time() const {
            return std::chrono::duration<uint64_t, std::nano>(time_running);
        }

        double get_ipc() const noexcept {
            uint64_t cpu_cycles = get_cpu_cycles();
            uint64_t instructions = get_instructions();
            return double(instructions) / double(cpu_cycles);
        }

        double get_cpi() const noexcept {
            uint64_t cpu_cycles = get_cpu_cycles();
            uint64_t instructions = get_instructions();
            return double(cpu_cycles) / double(instructions);
        }

        double get_l1d_missrate() const noexcept {
            uint64_t cache_references = get_l1d_references();
            uint64_t cache_misses = get_l1d_misses();
            return double(cache_misses) / double(cache_references);
        }

        double get_l1d_hitrate() const noexcept {
            return 1.0 - get_l1d_missrate();
        }

        double get_l1i_missrate() const noexcept {
            uint64_t cache_references = get_l1i_references();
            uint64_t cache_misses = get_l1i_misses();
            return double(cache_misses) / double(cache_references);
        }

        double getL1IHitRate() const noexcept {
            return 1.0 - get_l1i_missrate();
        }

        double get_branch_missrate() const noexcept {
            uint64_t branch_references = get_branch_instructions();
            uint64_t branch_misses = get_branch_misses();
            return double(branch_misses) / double(branch_references);
        }

        double get_branch_hitrate() const noexcept {
            return 1.0 - get_branch_missrate();
        }

        double get_mpki(uint64_t misses) const noexcept {
