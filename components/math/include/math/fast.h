#ifndef CHROMA_MATH_FAST_H
#define CHROMA_MATH_FAST_H
...
#if defined(__ARM_NEON) && defined(__aarch64__)
inline MATH_PURE uint8_t  qadd(uint8_t a, uint8_t b)  noexcept { return vuqaddb_s8(a, b); }
inline MATH_PURE uint16_t qadd(uint16_t a, uint16_t b) noexcept { return vuqaddh_s16(a, b); }
inline MATH_PURE uint32_t qadd(uint32_t a, uint32_t b) noexcept { return vuqadds_s32(a, b); }

inline MATH_PURE uint8_t  qsub(uint8_t a, uint8_t b)  noexcept { return vqsubb_s8(a, b); }
inline MATH_PURE uint16_t qsub(uint16_t a, uint16_t b) noexcept { return vqsubh_s16(a, b); }
inline MATH_PURE uint32_t qsub(uint32_t a, uint32_t b) noexcept { return vqsubs_s32(a, b); }
#else
