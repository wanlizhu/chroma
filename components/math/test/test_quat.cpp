...
TEST_F(QuatTest, ArithmeticFunc) {
    ...
    quat qr = quat::from_axis_angle(double3(0, 0, 1), M_PI / 2);
    EXPECT_EQ(mat4(qr).to_quaternion(), qr);
    EXPECT_EQ(1_i, mat4(1_i).to_quaternion());
    ...
    quat qa = quat::from_axis_angle(double3(0, 0, 1), 0);
    quat qb = quat::from_axis_angle(double3(0, 0, 1), M_PI / 2);
    quat qs = slerp(qa, qb, 0.5);
    qr = quat::from_axis_angle(double3(0, 0, 1), M_PI / 4);
...
TEST_F(QuatTest, MultiplicationExhaustive) {
    ...
    for (size_t i = 0; i < (1024 * 1024); ++i) {
        ...
        quat a = quat::from_axis_angle(axis_a, angle_a);

        double3 axis_b = normalize(double3(rand_gen(), rand_gen(), rand_gen()));
        double angle_b = rand_gen();
        quat b = quat::from_axis_angle(axis_b, angle_b);
